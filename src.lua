--[[
    ...
    Architecture:
        ...
        ├── Modal/Dialog       (Screen overlay dialog popup boxes)
...
local function buildDropdown(parent, theme, options)
    ...
    local function filterItems()
        ...
        for _, btn in ipairs(itemFrames) do
            local itemText = btn:FindFirstChildOfClass("TextLabel").Text:lower()
...
local function buildAccordion(parent, theme, options)
    ...
    function Accordion:Toggle(opts) return buildToggle(content, theme, opts) end
    function Accordion:Slider(opts) return buildSlider(content, theme, opts) end
    function Accordion:Button(opts) return buildButton(content, theme, opts) end
    function Accordion:Input(opts) return buildInput(content, theme, opts) end
    function Accordion:Dropdown(opts) return buildDropdown(content, theme, opts) end
    function Accordion:Keybind(opts) return buildKeybind(content, theme, opts) end
    function Accordion:ColorPicker(opts) return buildColorPicker(content, theme, opts) end
    function Accordion:Progress(opts) return buildProgress(content, theme, opts) end
    function Accordion:Label(opts) return buildLabel(content, theme, opts) end
    function Accordion:Paragraph(opts) return buildParagraph(content, theme, opts) end
...
local function buildSection(parent, theme, options)
    ...
    function Section:Toggle(opts) return wrapElement(buildToggle(content, theme, opts), true) end
    function Section:Slider(opts) return wrapElement(buildSlider(content, theme, opts), true) end
    function Section:Button(opts) return wrapElement(buildButton(content, theme, opts), true) end
    function Section:Input(opts) return wrapElement(buildInput(content, theme, opts), true) end
    function Section:Dropdown(opts) return wrapElement(buildDropdown(content, theme, opts), true) end
    function Section:Keybind(opts) return wrapElement(buildKeybind(content, theme, opts), true) end
    function Section:ColorPicker(opts) return wrapElement(buildColorPicker(content, theme, opts), true) end
    function Section:Progress(opts) return wrapElement(buildProgress(content, theme, opts), true) end
    function Section:Image(opts) return wrapElement(buildImage(content, theme, opts), true) end
    function Section:Accordion(opts) return wrapElement(buildAccordion(content, theme, opts), true) end
    function Section:Label(opts) return wrapElement(buildLabel(content, theme, opts), true) end
    function Section:Paragraph(opts) return wrapElement(buildParagraph(content, theme, opts), true) end
    ...
    function Section:Collapsible(state)
        setCollapsed(state)
    end
...
local function buildPage(parent, theme)
    ...
    function Page:Section(opts) return buildSection(inner, theme, opts) end
    function Page:Toggle(opts) return buildToggle(inner, theme, opts) end
    function Page:Slider(opts) return buildSlider(inner, theme, opts) end
    function Page:Button(opts) return buildButton(inner, theme, opts) end
    function Page:Input(opts) return buildInput(inner, theme, opts) end
    function Page:Dropdown(opts) return buildDropdown(inner, theme, opts) end
    function Page:Keybind(opts) return buildKeybind(inner, theme, opts) end
    function Page:ColorPicker(opts) return buildColorPicker(inner, theme, opts) end
    function Page:Progress(opts) return buildProgress(inner, theme, opts) end
    function Page:Image(opts) return buildImage(inner, theme, opts) end
    function Page:Accordion(opts) return buildAccordion(inner, theme, opts) end
    function Page:Label(opts) return buildLabel(inner, theme, opts) end
    function Page:Paragraph(opts) return buildParagraph(inner, theme, opts) end
    function Page:Divider(opts) return buildDivider(inner, theme, opts) end
    ...
    return {
        ...
        SelectTab   = function(self, idx)
            if tabItems[idx] then
                tabItems[idx].btn.MouseButton1Click:Fire()
            end
        end,
    }
end
...
local function createWindow(options)
            ...
            if flags[flag] then
                ...
                elseif el._type == "ColorPicker" then
                    if type(val) == "table" and #val == 3 then
                        el:Set(Color3.new(val[1], val[2], val[3]))
                    end
                elseif el.Set then
                    el:Set(val)
                end
    ...
    function Window:Page(index) return pages[index or 1] end
    function Window:Tab(index) return pages[index or 1] end
    function Window:Notify(opts)
        opts = opts or {}
        opts._theme = theme
        return notify(opts, theme)
    end
    function Window:Modal(opts) return modal(opts) end
    ...
    function Window:SetScale(scaleValue) uiScale.Scale = scaleValue end
    function Window:Open() showWindow() end
    function Window:Close() hideWindow() end
    function Window:Toggle() if isVisible then hideWindow() else showWindow() end end
    function Window:IsVisible() return isVisible end
    function Window:RegisterFlag(flag, element) flags[flag] = element end
...
function ArcUI:CreateWindow(options) return createWindow(options) end
function ArcUI:Notify(options) return notify(options, Themes[options.Theme or "Obsidian"]) end
function ArcUI:AddTheme(name, themeTable) Themes[name] = themeTable return themeTable end
function ArcUI:GetTheme(name) return Themes[name] end
function ArcUI:Section(parent, theme, options) return buildSection(parent, theme or Themes.Obsidian, options) end
function ArcUI:Tooltip(instance, text, theme) addTooltip(instance, text, theme or Themes.Obsidian) end
