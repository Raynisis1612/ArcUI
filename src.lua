...
local function hex(h)
    ...
    return Color3.fromRGB(
        tonumber(hsub(1,2),16),
        tonumber(hsub(3,4),16),
        tonumber(hsub(5,6),16)
    )
end
...
local function tween(obj, info, props)
    local t = TweenServiceCreate(obj, info, props)
    tPlay()
    return t
end
...
local function springTo(object, property, target, stiffness, damping, callback)
    ...
    local objId = (typeof(object) == Instance) and objectGetDebugId() or tostring(object)
    ...
    local connection
    connection = RunService.HeartbeatConnect(function(dt)
...
-- ─────────────────────────────────────────────────────────
local ICONS = {
    home          = rbxassetid7733715400,
    settings      = rbxassetid7733960895,
    search        = rbxassetid7734053986,
    close         = rbxassetid7734065065,
    chevron_right = rbxassetid7734068505,
    chevron_down  = rbxassetid7734070219,
    check         = rbxassetid7734073286,
    star          = rbxassetid7734076428,
    lock          = rbxassetid7734079134,
    eye           = rbxassetid7734081920,
    palette       = rbxassetid7734084544,
    key           = rbxassetid7734087124,
    slider_h      = rbxassetid7734090255,
    toggle_right  = rbxassetid7734093020,
    info          = rbxassetid7734095685,
    alert         = rbxassetid7734098210,
    trash         = rbxassetid7734100892,
    edit          = rbxassetid7734103502,
    plus          = rbxassetid7734106215,
    minus         = rbxassetid7734108828,
    user          = rbxassetid7734111555,
    bell          = rbxassetid7734114204,
    moon          = rbxassetid7734116840,
    sun           = rbxassetid7734119468,
    wifi          = rbxassetid7734122086,
    battery       = rbxassetid7734124640,
    arrow_right   = rbxassetid7734127175,
    arrow_left    = rbxassetid7734129774,
    copy          = rbxassetid7734132296,
    download      = rbxassetid7734134934,
    upload        = rbxassetid7734137495,
    grid          = rbxassetid7734140093,
    list          = rbxassetid7734142714,
    globe         = rbxassetid7734145237,
    code          = rbxassetid7734147838,
    terminal      = rbxassetid7734150438,
}
...
local function showTooltip(targetInstance, text, theme)
    if activeTooltip then activeTooltipDestroy() end
...
local function addTooltip(instance, text, theme)
    local delayThread
    instance.MouseEnterConnect(function()
        delayThread = task.delay(0.6, function()
            showTooltip(instance, text, theme)
        end)
    end)
    instance.MouseLeaveConnect(function()
...
local function ensureNotifGui()
    ...
    if not ok then gui.Parent = LocalPlayerWaitForChild(PlayerGui) end
...
local function notify(options, theme)
    ...
    if options.Actions and #options.Actions  0 then
        ...
        for _, act in ipairs(options.Actions) do
            ...
            actBtn.MouseButton1ClickConnect(function()
            ...
            actBtn.MouseEnterConnect(function()
                tween(actBtn, SPRING.fast, { BackgroundColor3 = theme.bg4 })
            end)
            actBtn.MouseLeaveConnect(function()
                tween(actBtn, SPRING.fast, { BackgroundColor3 = theme.bg3 })
            end)
        end
    end
    ...
    task.delay(dismissDelay, function()
        if card.Parent then
            tween(card, SPRING.medium, { Position = UDim2.new(1, 60, 0, 0), BackgroundTransparency = 1 })
            task.wait(0.4)
            if card.Parent then cardDestroy() end
        end
    end)
...
local function makeDraggable(frame, handle)
    ...
    handle.InputBeganConnect(function(inp)
    ...
    UserInputService.InputChangedConnect(function(inp)
        if not dragging then return end
        if inp.UserInputType == Enum.UserInputType.MouseMovement
        or inp.UserInputType == Enum.UserInputType.Touch then
            local scale = frameFindFirstChildOfClass(UIScale) and frameFindFirstChildOfClass(UIScale).Scale or 1
    ...
    UserInputService.InputEndedConnect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1
        or inp.UserInputType == Enum.UserInputType.Touch then
            dragging = false
        end
...
local function buildToggle(parent, theme, options)
    ...
    return {
        ...
        Destroy = function(self) rowDestroy() end,
        _type   = Toggle,
...
local function buildSlider(parent, theme, options)
    ...
    local function inputToValue(inputX)
        ...
        local scale = containerFindFirstAncestorOfClass(ScreenGui) and containerFindFirstAncestorOfClass(ScreenGui)FindFirstChildOfClass(UIScale) and containerFindFirstAncestorOfClass(ScreenGui)FindFirstChildOfClass(UIScale).Scale or 1
        local pct   = clamp(((inputX - apos)  scale)  (asize  scale), 0, 1)
        return min + pct  (max - min)
    end

    if not locked then
        hitbox.InputBeganConnect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1
            or inp.UserInputType == Enum.UserInputType.Touch then
                isDragging = true
    ...
    trackBgGetPropertyChangedSignal(AbsoluteSize)Connect(function()
    ...
    return {
        ...
        Destroy = function(self) containerDestroy() end,
        _type   = Slider,
...
local function buildButton(parent, theme, options)
    ...
    local function setLoading(loading)
        ...
        if loading then
            btn.Active = false
            if not btnGetAttribute(OldText) then
                btnSetAttribute(OldText, lbl.Text)
            end
            lbl.Text = Please wait...
            local spinner = btnFindFirstChild(Spinner)
            if not spinner then
            ...
            if spinConn then spinConnDisconnect() end
            local rot = 0
            spinConn = RunService.RenderSteppedConnect(function(dt)
                rot = (rot + 180  dt) % 360
                spinner.Rotation = rot
            end)
        else
            btn.Active = true
            local oldText = btnGetAttribute(OldText)
            if oldText then lbl.Text = oldText end
            local spinner = btnFindFirstChild(Spinner)
            if spinner then spinner.Visible = false end
            if iconImg then iconImg.Visible = true end
            if spinConn then spinConnDisconnect(); spinConn = nil end
        end
    end
...
local function buildDropdown(parent, theme, options)
    ...
    local function buildItems()
        for _, f in ipairs(itemFrames) do fDestroy() end
        itemFrames = {}
        listFrameClearAllChildren()
        ...
        for _, item in ipairs(items) do
            ...
            row.MouseButton1ClickConnect(function()
                if multi then
    ...
    local function filterItems()
        local query = searchBox and searchBox.Textlower() or 
        local visibleCount = 0
        for _, btn in ipairs(itemFrames) do
            local itemText = btnFindFirstChildOfClass(TextLabel).Textlower()
            if itemTextfind(query, 1, true) then
                btn.Visible = true
    ...
    if searchBox then
        searchBoxGetPropertyChangedSignal(Text)Connect(filterItems)
    end
...
local function buildKeybind(parent, theme, options)
    ...
    if not locked then
        hitbox.MouseButton1ClickConnect(function()
            ...
            local conn
            conn = UserInputService.InputBeganConnect(function(inp, gp)
        ...
        UserInputService.InputBeganConnect(function(inp, gp)
...
local function buildColorPicker(parent, theme, options)
    ...
    quadrant.InputBeganConnect(function(inp)
    ...
    hueSlider.InputBeganConnect(function(inp)
        if locked then return end
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            isDraggingHue = true
...
local function buildProgress(parent, theme, options)
    ...
    local valText = new(TextLabel, {
        ...
        AnchorPoint = Vector2.new(1, 0),
        BackgroundTransparency = 1,
        Text = tostring(math.floor((valuemax)100)) .. %,
        TextColor3 = theme.accent,
        Font = Enum.Font.GothamBold,
        TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Right,
...
local function buildParagraph(parent, theme, options)
    ...
    return {
        ...
        Set     = function(self, opts)
            if opts.Title   then cardFindFirstChildOfClass(TextLabel).Text = opts.Title end
            if opts.Content then body.Text = opts.Content end
        end,
        Destroy = function(self) cardDestroy() end,
        _type   = Paragraph,
    }
end
...
local function buildSection(parent, theme, options)
    ...
    if collapsible then
        ...
        headerHitbox.MouseButton1ClickConnect(function()
            setCollapsed(not isCollapsed)
        end)
    end
    ...
    function SectionDestroy() wrapperDestroy() end
...
local function buildTabBar(parent, theme, tabs, onChange)
    ...
    barGetPropertyChangedSignal(AbsoluteSize)Connect(recalculateWidths)
...
local function createWindow(options)
    ...
    restoreBtn.MouseButton1ClickConnect(restoreWindow)
    restoreBtn.MouseEnterConnect(function() tween(restorePill, SPRING.fast, { BackgroundColor3 = theme.bg2 }) end)
    restoreBtn.MouseLeaveConnect(function() tween(restorePill, SPRING.fast, { BackgroundColor3 = theme.bg1 }) end)
    ...
    do
        ...
        resizeHandle.InputBeganConnect(function(inp)
    ...
    local function saveConfig()
        ...
        pcall(writefile, configPath, HttpServiceJSONEncode(data))
    end
    ...
    local function loadConfig()
        ...
        local ok2, data = pcall(function() return HttpServiceJSONDecode(raw) end)
    ...
    local function modal(options)
        ...
        for _, act in ipairs(mActions) do
            ...
            actBtn.MouseEnterConnect(function() tween(actBtn, SPRING.fast, { BackgroundTransparency = 0.1 }) end)
            actBtn.MouseLeaveConnect(function() tween(actBtn, SPRING.fast, { BackgroundTransparency = 0 }) end)
        end
    ...
    function WindowDestroy()
        screenGuiDestroy()
        dropGuiDestroy()
        if NotificationGui then NotificationGuiDestroy() end
    end
